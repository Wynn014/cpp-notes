...
int main() {
    ...
    cout <<"============================="<< endl;
    cout <<"   TIP GAMING LOGIN SYSTEM  " << endl;
    cout <<"============================="<< endl;
    // ask first if staff or customer before going into the login part
    do {
        std::cout << "Are you a:"<<endl;
        std::cout << "[1] Staff / Owner"<<endl;
    ...
    if (userType == 1) {
        ...
        while (attempts < 3) {
            ...
            if (username == "Supervisor" && password == "Supervisor123") {
                role = "Supervisor";
                std::cout << "\nLogin successful! Logged in as SUPERVISOR." << std::endl << "\n";
                break;
            } else if(username == "InventoryAdmin" && password == "Admin123") {
                role = "InventoryAdmin";
                std::cout << "\nLogin successful! Logged in as INVENTORY ADMIN." << std::endl << "\n";
                break;
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2:
                if(role == "Supervisor" || role == "Cashier" || role == "Customer") {
                    processSale(ids, names, inventory, itemCount, totalTransactions, totalRevenue);
                } else {
                    std::cout << "Access denied. Only Cashier, Customer, and Supervisor can process sales.\n";
                }
                break;


            case 3:
                 if (role == "InventoryAdmin" || role == "Supervisor") {
                    restockProduct(ids, names, inventory, itemCount);
                 } else {
                    std::cout << "Access denied. Only Inventory Admin and Supervisor can restock products.\n";
                 }
                 break;
            ...
            case 4:
                if (role == "InventoryAdmin" || role == "Supervisor") {
                    ...
                    std::cout << "Access denied. Only Inventory Admin and Supervisor can add new products.\n";
                }
                break;


            case 5:
                if (role == "Supervisor" || role == "InventoryAdmin" || role == "Cashier") {
